...
function EncodingParametersInURLs() {
  ...
  const fetchSum = async (a, b) => {
    const response = await axios.get(`${LAB_BASE}/add/${a}/${b}`);
    setResult(response.data);
  };
  const fetchSubtraction = async (a, b) => {
    const response = await axios.get(`${LAB_BASE}/subtract/${a}/${b}`);
    setResult(response.data);
  };
  ...
  return (
    <div>
      ...
      <a href={`${LAB_BASE}/add/${a}/${b}`} className="btn btn-primary">
        Add {a} + {b}
      </a>
      <a href={`${LAB_BASE}/subtract/${a}/${b}`} className="btn btn-danger">
        Substract {a} - {b}
      </a>
